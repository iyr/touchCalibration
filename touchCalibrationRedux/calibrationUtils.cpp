...
void doCalibrate(
  ...
  if (currStep != prevStep) {
    drawCalibrate(tft, currStep, calibPoints);
    prevStep = currStep;
  }

  // Get, debounce touch input
  static bool prevTouch = false;
  bool currTouch = ts->touched();
  ...
  if (currStep == 8) {
    ...
    minXTS = mean(minXcalc, 4);
    minYTS = mean(minYcalc, 4);
    maxXTS = mean(maxXcalc, 4);
    maxYTS = mean(maxYcalc, 4);
